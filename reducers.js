 ...
 * test, and debug. And you can compose reducers, which makes it easy to implement simple
 * reducers that are concerned with only a portion of the overall application state.
 *
 * Because application state is immutable and because reducers are pure functions with
 * no side effects.
  *
 * Redux is a predictable state container: Given any state and any action, you can predict
 * the next state of the application with absolute certainty.
 *
 * That predictable element of Redux is the killer feature that opens the door to infinite
 * undo/redo and live-editing time travel.
 *
 *
